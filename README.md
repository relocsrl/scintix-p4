...
| Example | What it demonstrates | Console output |
| ------- | -------------------- | -------------- |
| [rm-cmp4_hello](examples/rm-cmp4_hello) | Minimal "Hello World" starter — verify your toolchain and board | UART0 / USB Serial-JTAG |
| [rm-cmp4_eth_iperf](examples/rm-cmp4_eth_iperf) | Ethernet throughput (iperf) over the on-board KSZ8091 PHY | USB Serial/JTAG |
| [rm-cmp4_eth_iperf_u0](examples/rm-cmp4_eth_iperf_u0) | Same as above, console on the module's own header | UART0 (6-pin connector) |
| [rm-cmp4_wifi_iperf](examples/rm-cmp4_wifi_iperf) | Wi-Fi throughput (iperf) via the ESP32-C6 companion (`esp_hosted`) | USB Serial/JTAG |
| [rm-cmp4_wifi_iperf_u0](examples/rm-cmp4_wifi_iperf_u0) | Same as above, console on the module's own header | UART0 (6-pin connector) |
| [rm-cmp4_usb_msc](examples/rm-cmp4_usb_msc) | USB host — mount and benchmark a USB flash drive | UART0 (6-pin connector) |
...
1. **Install ESP-IDF.** The `rm-cmp4_*` examples target **ESP-IDF v6.0.1**; the `esp_brookesia_phone` example targets **v5.5.4**. Follow the [ESP-IDF Programming Guide](https://docs.espressif.com/projects/esp-idf/en/latest/esp32p4/get-started/index.html) for setup.
